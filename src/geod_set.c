...
GEOD_init(int argc, char **argv, GEODESIC_T *GEODESIC)
{
  paralist *start = 0, *curr = 0;
	...
	if (pj_param(NULL,start, "tlat_1").i) {
		double del_S;
#undef f
                GEODESIC->p1.u = pj_param(NULL,start, "rlat_1").f;
 		GEODESIC->p1.v = pj_param(NULL,start, "rlon_1").f;
		if (pj_param(NULL,start, "tlat_2").i) {
 			GEODESIC->p2.u = pj_param(NULL,start, "rlat_2").f;
 			GEODESIC->p2.v = pj_param(NULL,start, "rlon_2").f;
 			geod_inv(GEODESIC);
 			geod_pre(GEODESIC);
 		} else if ((GEODESIC->DIST = pj_param(NULL,start, "dS").f)) {
 			GEODESIC->ALPHA12 = pj_param(NULL,start, "rA").f;
 			geod_pre(GEODESIC);
 			geod_for(GEODESIC);
		} else emess(1,"incomplete geodesic/arc info");
 		if ((GEODESIC->n_alpha = pj_param(NULL,start, "in_A").i) > 0) {
 			if (!(GEODESIC->del_alpha = pj_param(NULL,start, "rdel_A").f))
				emess(1,"del azimuth == 0");
		} else if (del_S = fabs(pj_param(NULL,start, "ddel_S").f)) {
 			GEODESIC->n_S = GEODESIC->DIST / del_S + .5;
 		} else if ((GEODESIC->n_S = pj_param(NULL,start, "in_S").i) <= 0)
